...
def calc(tparams, options, h, t, r):
    if options['distance'] == 'l1':
        return tensor.sum(tensor.abs_(h + r - t), axis=1)
    elif options['distance'] == 'l2':
        return tensor.sum(tensor.sqr(h + r - t), axis=1)
    else:
        raise NotImplementedError('Illegal distance measure.')


def build_tc_model(tparams, options, ttparams=None):
    if ttparams == None:
        ttparams = tparams
    trng = RandomStreams(SEED)
    ...
    ent_emb = ent_emb.reshape([n_samples, 2, options['edim']])

    # relation load
    # Naive approach
    rel_emb = ttparams['rel_emb'][r.flatten()]
    rel_emb = rel_emb.reshape([n_samples, options['edim']])

    text = tensor.matrix('text', dtype='int64')
    mask = tensor.matrix('text_mask', dtype=config.floatX)
    n_timesteps = text.shape[0]
    # text input shape : seqth lenth, batch_size * 2

    text_emb = tparams['word_emb'][text.flatten()]
    text_emb = text_emb.reshape([n_timesteps, n_samples, options['wdim']])

    if options['model_name'] == 'cbow' or options['model_name'] == 'cbow_gate' or options[
        'model_name'] == 'cbow_gate_pro':
        ...
        h_text, t_text = end_step[0:r.shape[0]], end_step[r.shape[0]: r.shape[0] * 2]

    elif options['model_name'] == 'lstm' or options['model_name'] == 'lstm_gate' or options[
        'model_name'] == 'lstm_gate_pro':
    ...
    elif options['model_name'] == 'lstm_att' or options['model_name'] == 'lstm_att_gate_pro' or options[
        'model_name'] == 'lstm_att_gate_pro':
        from module import attention, lstm
        rt_text = lstm(tparams, text_emb, options, mask=mask, in_dim=options['wdim'], out_dim=options['edim'])

        h_text, t_text = rt_text[:, 0:r.shape[0], :], rt_text[:, r.shape[0]: r.shape[0] * 2, :]

        h_text = attention(tparams, h_text, rel_emb, options, prefix='attention',
                           mask=mask[:, 0:r.shape[0]])
        h_text = dropout_layer(h_text, use_noise, trng)

        t_text = attention(tparams, t_text, rel_emb, options, prefix='attention',
                           mask=mask[:, r.shape[0]:r.shape[0] * 2])
        t_text = dropout_layer(t_text, use_noise, trng)
    else:
        raise NotImplementedError

    # h + r - t
    cost_ori = calc(tparams, options, ent_emb[:, 0, :], ent_emb[:, 1, :], rel_emb)

    # h_rnn +r -t
    cost_h_text = calc(tparams, options, h_text, ent_emb[:, 1, :], rel_emb)

    # h+r-rnn_t
    cost_t_text = calc(tparams, options, ent_emb[:, 0, :], t_text, rel_emb)

    # h_rnn + r - t_rnn
    cost_mul_text = calc(tparams, options, h_text, t_text, rel_emb)

    # gate
    f_loss_cost_gate, cost_gate_mean = None, None
    if options['model_name'].endswith('gate'):
        gate_emb = tparams['gate_emb'][ht.flatten()]
        gate_emb = gate_emb.reshape([n_samples, 2, options['edim']])
        sig_gate = tensor.nnet.sigmoid(gate_emb)
        gated_state = sig_gate * ent_emb + (1 - sig_gate) * (
            end_step.reshape((2, n_samples, options['edim']))).dimshuffle([1, 0, 2])

        cost_gate = calc(tparams, options, gated_state[:, 0, :],
                         gated_state[:, 1, :], rel_emb)
    ...
    return use_noise, ht, r, y, text, mask, \
