...
def handle_argv(argv):
    ...
    train(margin=margin, distance=distance, max_len=max_len,
          ...
          lrate=lr, lrtxt=lrtxt, data=data)
...
def train(
    ...
    fix_list = {}
    fix_list['ent_emb'] = True
    fix_list['rel_emb'] = True
    ...
    if options['model_name'].find('att') == -1:
        test_text, test_mask, __, test_end_step, alpha = build_test(tparams, options, ttparams=None)
        f_test = theano.function(inputs=[test_text, test_mask], outputs=[test_end_step, alpha])

    elif (options['model_name'].find('att') != -1) and (options['data'].find('fb') != -1):
        test_text, test_mask, rel, test_end_step = build_test(tparams, options, ttparams=None, relation_vec=False)
        f_test = theano.function(inputs=[test_text, test_mask, rel], outputs=[test_end_step])

    elif options['model_name'].find('att') != -1:
        test_text, test_mask, rel, test_end_step = build_test(tparams, options, ttparams=None)
        f_test = theano.function(inputs=[test_text, test_mask, rel], outputs=[test_end_step])
    ######
    # test for gene test

    generate_test_text(tparams, options, f_test, 0, ttparams)
    ...
    try:
        for eidx in xrange(max_epochs):
            ...
            for i in xrange(batch_num):
                ...
                if numpy.mod(uidx, 4 * save_freq) == 0:
                    print('--SAVING TEST--')
