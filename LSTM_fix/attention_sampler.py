...
def handle_argv(argv):
    ...
    for o, a in opts:
        if o == '--marg':
            ...
            optim = str(a)
    test(margin=margin, distance=distance, max_len=max_len,
         model_name=model, load_param=load_param, batch_size=batch_size,
         loss_code=loss_code, edim=edim, wdim=wdim,
         lrate=lr, lrtxt=lrtxt, data=data, fix=fix, optim=optim)
...
def test(
    ...
    saveto = ('%s%s%s%s%s_%s_%s_%s_%s_%s_%s_%s.pkl') % (
    ...
    # if model_name.startswith('lstm_att'):
    ...
    elif model_name.startswith('cbow'):
    ...
    if fix == True:
        ...
        print(
            'Real: word_emb: %s ent_emb: %s  Assumption: %s %s' % (
                tparams['word_emb'].get_value().shape[1], ttparams['ent_emb'].get_value().shape[1],
                options['wdim'], options['edim']))
        assert tparams['word_emb'].get_value().shape[1] == options['wdim']
        assert ttparams['ent_emb'].get_value().shape[1] == options['edim']
    else:
        tparams = init_tparams(params)
        print(
            'Real: word_emb: %s ent_emb: %s  Assumption: %s %s' % (
                tparams['word_emb'].get_value().shape[1], tparams['ent_emb'].get_value().shape[1],
                options['wdim'], options['edim']))
        assert tparams['word_emb'].get_value().shape[1] == options['wdim']
        assert tparams['ent_emb'].get_value().shape[1] == options['edim']
    ...
    # Prepare test data
    # TODO
    # lstm or cbow. no gate. no att.
    if options['model_name'].find('att') == -1 and options['model_name'].find('gate') == -1:
        test_text, test_mask, __, test_end_step, __ = build_test(tparams, options, ttparams)
        f_test = theano.function(inputs=[test_text, test_mask], outputs=[test_end_step])

    # no att . with gate.
    elif options['model_name'].find('att') == -1 and options['model_name'].find('gate') != -1:
        test_text, test_mask, __, test_end_step, alpha = build_test(tparams, options, ttparams)
        f_test = theano.function(inputs=[test_text, test_mask], outputs=[test_end_step, alpha])


    # with att. no gate.
    elif options['model_name'].find('gate') == -1 and options['model_name'].find('att') != -1:
        test_text, test_mask, rel, test_end_step, __ = build_test(tparams, options, ttparams, relation_vec=False)
        f_test = theano.function(inputs=[test_text, test_mask, rel], outputs=[test_end_step], on_unused_input='ignore')


    # with att and gate
    elif options['model_name'].find('gate') != -1 and options['model_name'].find('att') != -1:
        test_text, test_mask, rel, test_end_step, alpha = build_test(tparams, options, ttparams, relation_vec=False)
        f_test = theano.function(inputs=[test_text, test_mask, rel], outputs=[test_end_step, alpha])
    ...
    generate_test_text(tparams, options, f_test, 0, ttparams, build_test)

    if model_name == 'transe':
        (ht, r, f_loss_cost_ori, cost_ori) = build_model(tparams, options)
    elif model_name.endswith('gate') or model_name.endswith('gate_pro') or model_name.find('gate') != -1:
    ...
    elif model_name == 'lstm' or model_name == 'lstm_att' or model_name == 'cbow' or model_name == 'cbow_att':
        (use_noise, ht, r, text, mask,
         f_loss_cost_ori, cost_ori,
         f_loss_cost_h_text, cost_h_text,
         f_loss_cost_t_text, cost_t_text,
         f_loss_cost_mul_text, cost_mul_text,
         __, __) = build_model(tparams, options, ttparams)
    else:
        raise NotImplementedError
    ...
    try:
        for eidx in xrange(max_epochs):
            ...
            for i in xrange(batch_num):
                ...
                # if numpy.isnan(cost_h_text) or

                if numpy.mod(uidx, disp_freq) == 0:
                    if options['model_name'].find('att') != -1:
                        pass
                        # print(tparams['attention_U'].get_value().max())
                        # print(tparams['attention_U'].get_value().mean())
                        #
                        # print(tparams['attention_W'].get_value().max())
                        # print(tparams['attention_W'].get_value().mean())

                        # print(tparams['attention_V'].get_value().max())
                        # print(tparams['attention_V'].get_value().mean())
                        # print(tparams['attention_W'].get_value().max())
                    ...
                    if options['loss4']:
                        print 'Epoch\t', eidx, 'Update\t', uidx, 'Cost_g\t', cost_gate
                        if model_name.endswith('gate'):
                            print ' alpha mean\t', (
                                tparams['gate_emb'].get_value()).mean()
                        elif model_name.endswith('gate_pro'):
                            pass
                ...
                if numpy.mod(uidx, 5 * save_freq) == 0:
                    print('--SAVING TEST--')
                    generate_test_text(tparams, options, f_test, eidx, ttparams, build_test)
    ...
    end_time = time.time()
    generate_test_text(tparams, options, f_test, 9999, ttparams, build_test)
